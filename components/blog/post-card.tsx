        ...
        <Link href={`/${locale}/blog/${post.slug}`} className="group block">
            <article className="flex flex-col gap-4 mb-2">
                ...
                <div className="overflow-hidden rounded-lg bg-muted">
                    <Image
                        src={`${process.env.HOST}${post.image.formats.medium.url || post.image.url}`}
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        <Avatar className="h-6 w-6">
                            <AvatarImage src={`${process.env.HOST}${post.author?.picture.formats.thumbnail.url || post.author?.picture.url}`} alt={author_name || 'Article Author'} />
